...
class ClientManager:
    ...
    @staticmethod
    def get_config(course_name: str) -> dict[str, Any]:
    ...
    @classmethod
    async def get_client(cls, course_name: str) -> PostgreSQLDB:
        async with cls._lock:
            if cls._instances["db"] is None:
                config = ClientManager.get_config(course_name = course_name)
...
class PGKVStorage(BaseKVStorage):
    ...
    async def initialize(self, course_name: str):
        if self.db is None:
            self.db = await ClientManager.get_client(course_name = course_name)
